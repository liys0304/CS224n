...
\end{equation*}
The $y_i$ is one-hot code, so it equals 1 only at the case when $i=o$, and other cases is 0, so it equals to the naive-softmax loss.
\end{answer}
\end{shaded}
...
i. \\
    \begin{align*}
    \bm{J}_{\text{naive-softmax}}(\bm v_c, o, \bm U) = -\log(P(O=o|C=c))  \\
        P(O=o|C=c) = \frac{\exp(\bm u_o^T\bm v_c)}{\sum_{w \in V} \exp(\bm u_w^T\bm v_c)} \\
    \bm J_{\text{naive-softmax}} &= -\log \big( \frac{\exp(\bm u_o^T\bm v_c)}{\sum_{w \in V}\exp(\bm u_w^T\bm v_c)} \big) \\
    &= \log{\bigg(\sum_{w \in V}\exp(\bm u_w^T\bm v_c)\bigg)} - \log{\exp(\bm u_o^T\bm v_c)} \\
    \frac{\partial \bm J_{\text{naive-softmax}}}{\partial \bm v_{c}} &= 
    \sum_{w \in V}\frac{1}{\sum_{w \in V}\exp(\bm u_w^T\bm v_c)}\cdot \frac{\partial \exp(\bm u_w^T\bv_c)}{\partial \bm v_c} - \bm u_o \\
    &= \sum_{w \in V}\frac{\exp(\bm u_w^T\bm v_c)}{\sum_{w \in V}\exp(\bm u_w^T\bm v_c)} \cdot \bm u_w - \bm u_o \\
    &=\sum_{w \in V}\hat{\bm y}_w \cdot \bm u_w - \bm u_o\\
    &=\bm U(\hat{\bm y} - \bm y)
    \end{align*}
\end{answer}
\end{shaded}
...
\begin{align*}
		...
		&= \log{\bigg(\sum_{w \in V}\exp(\bm u_w^T\bm v_c)\bigg)} - \log{\exp(\bm u_o^T\bm v_c)} \\
		& \text{when $w=o$:} \\
		\frac{\partial \bm J_{\text{naive-softmax}}}{\partial \bm u_{w}} &= 
		\frac{1}{\sum_{w \in V}\exp(\bm u_w^T\bm v_c)}\cdot \frac{\partial \exp(\bm u_w^T\bm v_c)}{\partial \bm u_w} - \bm v_c \\
		&= (\hat{\bm y_o} - 1) \cdot \bm v_c \\
		& \text{when $w \ne o$:} \\
		\frac{\partial \bm J_{\text{naive-softmax}}}{\partial \bm u_{w}} &= 
		\frac{1}{\sum_{w \in V}\exp(\bm u_w^T\bm v_c)}\cdot \frac{\partial \exp(\bm u_w^T\bm v_c)}{\partial \bm u_w} \\
		&= \hat{\bm y_w} \cdot \bm v_c
	\end{align*}
\end{answer}
\end{shaded}
...
\begin{answer}
    \begin{align*}
		\frac{\partial \bm J_{\text{naive-softmax}}}{\partial \bm U} &= 
		\sum_{w \in V}\frac{1}{\sum_{w \in V}\exp(\bm u_w^T\bm v_c)}\cdot \frac{\partial \exp(\bm u_w^T\bm v_c)}{\partial \bm U} - \frac{\partial (\bm u_o^T\bm v_c)}{\partial \bm U} \\
		&= \sum_{w \in V}\frac{\exp(\bm u_w^T\bm v_c)}{\sum_{w \in V}\exp(\bm u_w^T\bm v_c)} \cdot \bm u_w - \bm u_o \\
		&=\sum_{w \in V}\hat{\bm y}_w \cdot \bm u_w - \bm u_o\\
		&=\bm U(\hat{\bm y} - \bm y)
	\end{align*}
...
\begin{answer}
    \begin{align*}
        ...
        &= \sigma(x)(\frac{1}{e^x(e^{-x}+1)}) \\
        &= \sigma^2(x)e^{-x} \\
        &= \sigma^2(x)\frac{1-\sigma(x)}{\sigma(x)} \\
        &= \sigma(x)(1-\sigma(x))
    \end{align*}
\end{answer}
\end{shaded}
...
\begin{align*}
    \bm J_{\text{neg-sample}}(\bm v_c, o, \bm U) &= -\log(\sigma(\bm u_o^\top \bm v_c)) - \sum_{s=1}^K \log(\sigma(-\bm u_{w_s}^\top \bm v_c)) \\
    \frac{\partial\bm J_{\text{neg-sample}}}{\partial \bm v_c} &= -\frac{1}{\sigma(\bm u_o^T\bm v_c)}\frac{\partial \sigma(\bm u_o^T\bm v_c)}{\partial\bm v_c} - \sum_{s=1}^K\frac{1}{\sigma(\bm u_w_s^T\bm v_c)}\frac{\partial \sigma(\bm u_w_s^T\bm v_c)}{\partial\bm v_c}\\
    &= -\frac{1}{\sigma(\bm u_o^T\bm v_c)}(\sigma(\bm u_o^T\bm v_c)(1-\sigma(\bm u_o^T\bm v_c))\cdot \bm u_o^T-\sum_{s=1}^K\frac{1}{\sigma(\bm u_w_s^T\bm v_c)}(\sigma(\bm u_w_s^T\bm v_c)(1-\sigma(\bm u_w_s^T\bm v_c))\cdot \bm u_w_s \\
    &=(\sigma(\bm u_o^T\bm v_c)-1)\cdot \bm u_o^T - \sum_{s=1}^K(\sigma(\bm u_w_s^T\bm v_c)-1)\cdot \bm u_w_s\\
    \frac{\partial\bm J_{\text{neg-sample}}}{\partial \bm u_o} &= -\frac{1}{\sigma(\bm u_o^T\bm v_c)}\frac{\partial \sigma(\bm u_o^T\bm v_c)}{\partial\bm u_o}\\
    &= -\frac{1}{\sigma(\bm u_o^T\bm v_c)}(\sigma(\bm u_o^T\bm v_c)(1-\sigma(\bm u_o^T\bm v_c))\cdot \bm v_c \\
    &=(\sigma(\bm u_o^T\bm v_c) - 1)\cdot \bm v_c \\
    \frac{\partial\bm J_{\text{neg-sample}}}{\partial \bm u_{w_s}} &= -\frac{1}{\sigma}\frac{\partial \sigma}{\partial \bm u_{w_s}} \\
    &= (1-\sigma)\bm v_c
\end{align*}
iii:    This loss function only have far less samples than the naive-softmax loss.
\end{answer}
\end{shaded}
...
\begin{answer}
\begin{align*}
    \text{when $w_s \ne o$, for a specific s,}\\
    \frac{\partial \bm J}{\partial \bm u_{w_s}} &= \frac{\partial}{\partial \bm u_{w_s}} (-\log(\sigma(-\bm u_{w_s}^T\bm v_c))) \\
    &=-\frac{1}{\sigma}\sigma(1-\sigma)(-\bm v_c)\\
    &=(1-\sigma)\bm v_c
\end{align*}
\end{answer}
\end{shaded}
...
\begin{answer}
\begin{align*}
    \frac{\partial \bm J_{\textrm{skip-gram}}(\bm v_c, w_{t-m},\ldots w_{t+m}, \bm U)} {\partial \bm U} &= \sum_{-m \le j\le m, j\ne0}\frac{\partial \bm J(\bm v_c, w_{t+j}, \bm U)}{\partial \bm U}\\
    \frac{\partial \bm J_{\textrm{skip-gram}}(\bm v_c, w_{t-m},\ldots w_{t+m}, \bm U)} {\partial \bm v_c}&=\sum_{-m \le j\le m, j\ne0}\frac{\partial \bm J(\bm v_c, w_{t+j}, \bm U)}{\partial \bm v_c}\\
    \frac{\partial \bm J_{\textrm{skip-gram}}(\bm v_c, w_{t-m},\ldots w_{t+m}, \bm U)} {\partial \bm v_w}\text{when $w\ne c$}&=\sum_{j:w_{t+j} = w}\frac{\partial \bm J(\bm v_c, w_{t+j}, \bm U)}{\partial \bm v_w}
\end{align*}
\end{answer}
\end{shaded}
